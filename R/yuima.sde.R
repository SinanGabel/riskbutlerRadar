...
#'
#' Warning: dependency in eulerOne()
#' ToDo: add heston model
#'
#' @param model (you3, yckls, yhdf_b2)
...
yuima.sde <- function(model = "you3", multivariate = FALSE, correlated = FALSE, data) {
  ...
  if (model == "you3" & multivariate == TRUE) {
    ...
    # dX = sigma^2/2·(beta - gamma·X/Sqrt(delta^2 + (X - mu)^2)·dt + sigma2·X·dW, Hyperbolic diffusion - B (dynamics of sand - the sand project, AAU); Iacus, 2011, Chicago R/Finance, April 29
  } else if (model == "yhdf_b2" & multivariate == TRUE) {
    b <- c("(p1.1^2)/2 * (p2.1 - p3.1 * x1 /sqrt(p4.1^2 + (x1 - p5.1)^2))", "(p1.2^2)/2 * (p2.2 - p3.2 * x2 /sqrt(p4.2^2 + (x2 - p5.2)^2))")
    e21 <- gsub("rho", rho, "p6.2 * x * rho")
    e22 <- gsub("rho", rho, "p6.2 * x * sqrt(1 - (rho)^2)")
    s <- matrix(c("p1.1", e21, 0, e22), 2, 2)
    slu <- jsonlite::fromJSON('{"start": {"p1.1": 0.1,"p2.1": 0.1,"p3.1": 0.2, "p4.1": 0.1, "p5.1": 0.1,"p6.1": 0.1, "p1.2": 0.1,"p2.2": 0.1,"p3.2": 0.2, "p4.2": 0.1, "p5.2": 0.1,"p6.2": 0.1}, "lower": {"p1.1": 0,"p2.1": 0,"p3.1": 0, "p4.1": 0, "p5.1": -100,"p6.1": 0, "p1.2": 0,"p2.2": 0,"p3.2": 0, "p4.2": 0, "p5.2": -100,"p6.2": 0}, "upper": {"p1.1": 1,"p2.1": 100,"p3.1": 1, "p4.1": 1, "p5.1": 100,"p6.1": 1, "p1.2": 1,"p2.2": 100,"p3.2": 1, "p4.1": 1, "p5.2": 100, "p6.2": 1}}')
    sol <- c("x1", "x2")

  } else if (model == "yhdf_b2") {
    b <- "(p1^2)/2 * (p2 - p3 * x /sqrt(p4^2 + (x - p5)^2))"
    s <- "p6 * x"
    slu <- jsonlite::fromJSON('{"start": {"p1": 0.1,"p2": 0.1,"p3": 0.2,"p4": 0.1,"p5": 0.1,"p6": 0.1}, "lower": {"p1": 0,"p2": 0,"p3": 0,"p4": 0,"p5": -100,"p6": 0}, "upper": {"p1": 1,"p2": 100,"p3": 1,"p4": 1,"p5": 100,"p6": 1}}')
    sol <- "x"
